...
```bash
git clone https://github.com/Aayushiii25/PulseIq.git
cd PulseIq
python3 -m venv .venv
...
### 2. Seed the Database (no API key needed)
...
This runs: **embeddings → clustering → sentiment analysis** on the seeded data.

To also fetch fresh news (requires a [NewsAPI key](https://newsapi.org/register)):
...
### 4. Generate Visualizations (PNG Output)

Generate/regenerate the high-quality Plotly data charts and save them directly as PNGs in the `images/` directory:
...
Here are the optimized charts generated by the ML pipeline running on the pre-generated financial news dataset. You can recreate these figures locally at any time by running:
```bash
python generate_graphs.py
```
